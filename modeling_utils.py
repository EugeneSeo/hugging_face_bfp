...
def _load_state_dict_into_model(model_to_load, state_dict, start_prefix):
    ...
    def load(module: nn.Module, state_dict, prefix=""):
        ...
        for name, child in module._modules.items():
            ...
            #     key_w = prefix + name + ".weight"
            #     key_b = prefix + name + ".bias"
            #     print("keys: key_w - {}, key_b = {}".format(key_w, key_b))
            #     if key_w in state_dict:
            #         print("weight loaded")
            #         child.weight = nn.Parameter(state_dict[key_w].clone().detach())
            #     if key_b in state_dict:
            #         print("bias loaded")
            #         child.bias = nn.Parameter(state_dict[key_b].clone().detach())
    ...
    #     # state_dict[key] = torch.round(state_dict[key]) # All parameters are 0 or 1
    #     if not os.path.isfile("./dummy_ckpt3/{}.npy".format(key)):
    #         # new_param = np.round(np.random.rand(*state_dict[key].shape)*2 - 1.0, 0)
    #         new_param = np.round(np.random.randint(2, size=state_dict[key].shape))
    #         np.save("./dummy_ckpt3/{}.npy".format(key), new_param)
    #     state_dict[key] = Tensor(np.load("./dummy_ckpt3/{}.npy".format(key)))
    ####################
...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, GenerationMixin, PushToHubMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], *model_args, **kwargs):
        ...
        * `low_cpu_mem_usage` algorithm:
        ...
        # print("line 2160: Model is defined")
        ...
        if from_tf:
            ...
            # print("line 2255: Loading pretrained model")
