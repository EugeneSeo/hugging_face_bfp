...
class BertSelfAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        ...
        if config._BFP_flag == False:
            ...
            # self.matmul = torch.matmul
        self.matmul_1 = None
        # torch.Size([8, 12, 64, 64]) torch.Size([8, 12, 64, 64])  B H max_len max_len
        # self.matmul_2 = CustomMatMul(torch.Size([8, self.num_attention_heads, 64, 64]), torch.Size([8, self.num_attention_heads, 64, 64]), config._USE_STOCHASTIC_ROUNDING, False, False, 100)
        self.matmul_2 = None
        self.config = config
    ...
    def forward(
        ...
        if self.matmul is not None:
            ...
            if self.matmul_1 is None:
                self.matmul_1 = CustomMatMul(query_layer.shape, key_layer.shape, self.config._USE_STOCHASTIC_ROUNDING, False, False, 100)
        ...
        if self.matmul is not None:
            ...
            if self.matmul_2 is None:
                ...
                self.matmul_2 = CustomMatMul(attention_probs.shape, value_layer.shape, self.config._USE_STOCHASTIC_ROUNDING, False, False, 100)
...
class BertSelfOutput(nn.Module):
    def __init__(self, config):
        ...
        if config._BFP_flag == False:
            ...
            global GLOBAL_ID
            self.dense = CustomLinear(config.hidden_size, config.hidden_size, bias=True, precision_flag=config.PrecisionFlag, global_id=GLOBAL_ID)
            GLOBAL_ID += 1
        self.LayerNorm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class BertIntermediate(nn.Module):
    def __init__(self, config):
        ...
        if config._BFP_flag == False:
            ...
            global GLOBAL_ID
            self.dense = CustomLinear(config.hidden_size, config.intermediate_size, bias=True, precision_flag=config.PrecisionFlag, global_id=GLOBAL_ID)
            GLOBAL_ID += 1
...
class BertOutput(nn.Module):
    def __init__(self, config):
        ...
        if config._BFP_flag == False:
            ...
            global GLOBAL_ID
            self.dense = CustomLinear(config.intermediate_size, config.hidden_size, bias=True, precision_flag=config.PrecisionFlag, global_id=GLOBAL_ID)
            GLOBAL_ID += 1
        self.LayerNorm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class BertLMPredictionHead(nn.Module):
    def __init__(self, config):
        ...
        if config._BFP_flag == False:
            ...
            global GLOBAL_ID
            self.decoder = CustomLinear(config.hidden_size, config.vocab_size, bias=False, precision_flag=config.PrecisionFlag, global_id=GLOBAL_ID)
            GLOBAL_ID += 1
...
class BertOnlyNSPHead(nn.Module):
    def __init__(self, config):
        ...
        if config._BFP_flag == False:
            ...
            global GLOBAL_ID
            self.seq_relationship = CustomLinear(config.hidden_size, 2, bias=True, precision_flag=config.PrecisionFlag, global_id=GLOBAL_ID)
            GLOBAL_ID += 1
...
class BertPreTrainingHeads(nn.Module):
    def __init__(self, config):
        ...
        if config._BFP_flag == False:
            ...
            global GLOBAL_ID
            self.seq_relationship = CustomLinear(config.hidden_size, config.vocab_size, bias=True, precision_flag=config.PrecisionFlag, global_id=GLOBAL_ID)
            GLOBAL_ID += 1
...
class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        if config._BFP_flag == False:
            ...
            global GLOBAL_ID
            self.classifier = CustomLinear(config.hidden_size, config.num_labels, bias=True, precision_flag=config.PrecisionFlag, global_id=GLOBAL_ID)
            GLOBAL_ID += 1
...
class BertForMultipleChoice(BertPreTrainedModel):
    def __init__(self, config):
        ...
        if config._BFP_flag == False:
            ...
            global GLOBAL_ID
            self.classifier = CustomLinear(config.hidden_size, 1, bias=True, precision_flag=config.PrecisionFlag, global_id=GLOBAL_ID)
            GLOBAL_ID += 1
...
class BertForQuestionAnswering(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if config._BFP_flag == False:
            ...
            global GLOBAL_ID
            self.qa_outputs = CustomLinear(config.hidden_size, config.num_labels, bias=True, precision_flag=config.PrecisionFlag, global_id=GLOBAL_ID)
            GLOBAL_ID += 1
...
#             # key_layer = key_layer.transpose(-1, -2)
#             if self.matmul_1 is None:
#                 self.matmul_1 = CustomMatMul(query_layer.shape, key_layer.transpose(-1, -2).shape, self.config._USE_STOCHASTIC_ROUNDING, False, False, 100)
#             # attention_scores = self.matmul_1.run(query_layer, key_layer.transpose(-1, -2))
#             attention_scores = self.matmul_1.run(query_layer, key_layer.transpose(-1, -2)).view(shape_spec[0], shape_spec[1], shape_spec[2], -1)
#             print("matmul error:", torch.sum(torch.abs(attention_scores - torch.matmul(query_layer, key_layer.transpose(-1, -2)))))
#         # [PROBLEM - SOLVED] 
#         '''
#             torch.matmul: supports (..., M, K) * (..., K, N) -> (..., M, N)
#             BfpGemm: supports (M, K) * (K, N) -> (M, N)
